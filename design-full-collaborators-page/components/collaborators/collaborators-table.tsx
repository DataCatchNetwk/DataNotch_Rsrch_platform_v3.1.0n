  ...
  const filtered = useMemo(() => {
    return collaborators.filter((item) => {
      ...
      const matchesSearch = !query
        ...
        || item.workspaces.some((workspace) => workspace.workspaceName.toLowerCase().includes(query))
      ...
      const matchesWorkspace =
        ...
        || item.workspaces.some((workspace) => workspace.workspaceName === filters.workspace)
          ...
          {filtered.length === 0 ? (
            ...
            filtered.map((item) => (
              <TableRow key={item.id} className="hover:bg-muted/30">
                ...
                <TableCell>
                  <Badge variant={item.status === "SUSPENDED" ? "destructive" : item.status === "ACTIVE" ? "default" : "secondary"}>
                    {item.status}
                  </Badge>
                </TableCell>
                <TableCell>
                ...
                <TableCell>
                  <div className="flex max-w-[280px] flex-wrap gap-1">
                    {item.workspaces.map((workspace) => (
                      <Badge key={workspace.workspaceId} variant="outline">

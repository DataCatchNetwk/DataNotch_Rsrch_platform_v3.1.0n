...
export function GlobalCollaboratorsPage() {
  ...
  return (
    <div className="space-y-6 p-6">
      <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
        <div className="space-y-1">
          ...
          <p className="text-sm text-muted-foreground max-w-3xl">
            Unified collaborator roster tied to workspace memberships, role permissions, invite lifecycle,
            and research access controls.
          </p>
        </div>
          ...
          <div className="rounded-2xl border bg-card p-5 shadow-sm">
            ...
            <div className="mt-3 space-y-3">
              {quickInsights.map((note) => (
                <div key={note} className="rounded-xl border bg-muted/30 p-3 text-sm text-muted-foreground">
                  {note}
                </div>
              ))}
            </div>
      ...
      <CollaboratorDetailDrawer open={detailOpen} onOpenChange={setDetailOpen} collaborator={selected} />
    </div>
  )
}

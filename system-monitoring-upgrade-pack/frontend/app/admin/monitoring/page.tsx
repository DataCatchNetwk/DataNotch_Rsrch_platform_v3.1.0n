      ...
      {loading ? <MonitoringLoading /> : error ? <MonitoringError message={error} onRetry={() => void load()} /> : overview && metrics && queue ? (
        <div className="space-y-6">
          <Card className="rounded-3xl border-0 shadow-sm">
            <CardContent className="flex flex-col gap-6 p-6 xl:flex-row xl:items-start xl:justify-between">
              <div>
                ...
                <h2 className="mt-2 text-3xl font-semibold tracking-tight text-slate-950">System Health Control Plane</h2>
                <p className="mt-3 text-sm text-slate-600">Live operational state for API, queues, workers, and services.</p>
              ...
              <div className="flex flex-wrap gap-2">
                <Button variant="outline" onClick={() => void runAction("refresh", refreshMonitoring)} disabled={actionLoading === "refresh"}>
                  <RefreshCw className="mr-2 h-4 w-4" />Refresh
                </Button>
                <Button variant="outline" onClick={() => void runAction("retry", retryFailedJobs)} disabled={actionLoading === "retry"}>
                  <RotateCcw className="mr-2 h-4 w-4" />Retry Failed Jobs
                </Button>
                <Button variant="destructive" onClick={() => void runAction("clear", clearMonitoringQueue)} disabled={actionLoading === "clear"}>
          ...
          <section className="grid gap-4 xl:grid-cols-3">
            {alerts.map((item) => (
              <div key={item.id} className={`rounded-2xl border p-4 shadow-sm ${alertToneClass(item.tone)}`}>
            ...
            <div className="space-y-6">
              <Card className="rounded-2xl border-0 shadow-sm">
                ...
                <CardContent className="grid gap-3">
                  {[
                    ...
                    <div key={String(label)} className="flex items-center justify-between rounded-2xl border bg-slate-50 p-4">
              ...
              <Card className="rounded-2xl border-0 shadow-sm">
                ...
                <CardContent className="grid gap-2">
                  <Button variant="outline" className="justify-start"><ServerCog className="mr-2 h-4 w-4" />Restart Worker Cluster</Button>
                  <Button variant="outline" className="justify-start"><ShieldCheck className="mr-2 h-4 w-4" />Run Health Check</Button>
                  <Button variant="outline" className="justify-start"><Download className="mr-2 h-4 w-4" />Export Monitoring Logs</Button>
                </CardContent>
              </Card>
            </div>
          </section>

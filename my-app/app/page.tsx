...
const googleSsoEnabled = process.env.NEXT_PUBLIC_GOOGLE_SSO_ENABLED === 'true';
...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    setSecureConnection(window.isSecureContext || window.location.protocol === 'https:' || isLocalhost);
    ...
    const checkNetworkRisk = async () => {
      try {
        ...
        if (data.blocked) {
          ...
          setVpnBlocked(false);
          setVpnMessage(data.checked === false ? 'Network risk check unavailable.' : 'Network check passed.');
        }
      } catch {
        if (!isMounted) return;
        setVpnBlocked(false);
  ...
  const handleGoogleSignIn = async () => {
    if (!googleSsoEnabled || loading || ssoLoading) return;
    setError('');
  ...
  const handleMicrosoftSignIn = async () => {
    if (!microsoftSsoEnabled || loading || ssoLoading) return;
    setError('');
      ...
      <div className="mx-auto grid h-[calc(100dvh-1rem)] w-full max-w-6xl grid-cols-1 items-stretch gap-3 lg:grid-cols-2 lg:gap-4">
        <div className="h-full overflow-y-auto rounded-3xl bg-white p-4 shadow-2xl md:p-5">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className={`flex items-center gap-1 text-sm ${secureConnection ? 'text-green-600' : 'text-amber-600'}`}>
              <Check className="h-4 w-4" /> {secureConnection ? 'Secure connection verified' : 'Connection security check unavailable'}
            </div>

            <div className={`text-sm ${vpnBlocked ? 'text-red-600' : 'text-gray-600'}`}>

...
export async function createSupportTicket(payload: CreateSupportTicketPayload) {
  ...
  const response = await fetch(`${(process.env.NEXT_PUBLIC_API_BASE_URL ?? 'http://localhost:4000').replace(/\/+$/, '')}/api/v1/support`, {
    method: 'POST',
    headers,
    body: formData,
  });
...
export async function replyToSupportTicket(ticketId: string, payload: { message: string; isInternal?: boolean; attachment?: File | null }) {
  ...
  const response = await fetch(`${(process.env.NEXT_PUBLIC_API_BASE_URL ?? 'http://localhost:4000').replace(/\/+$/, '')}/api/v1/support/${ticketId}/reply`, {
    method: 'POST',

...
import type { NotificationCategory, NotificationItem, NotificationStatus } from "@/types/notification"
...
export function NotificationsPage() {
  ...
  const visible = React.useMemo(() => {
    ...
    if (!q) return items
    return items.filter((item) => [item.title, item.message, item.category].join(" ").toLowerCase().includes(q))
  }, [items, search])
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        ...
        <Card className="rounded-3xl">
          <CardHeader className="pb-2">
            ...
            <CardTitle>{stats.critical}</CardTitle>
          </CardHeader>
        </Card>
        <Card className="rounded-3xl">
          <CardHeader className="pb-2">
            <CardDescription>Action Required</CardDescription>
            <CardTitle>{stats.actionRequired}</CardTitle>
          </CardHeader>
        </Card>
      </div>
        ...
        <CardContent className="flex flex-col gap-4 p-4 lg:flex-row lg:items-center lg:justify-between">
          <div className="flex flex-1 items-center gap-3">
            <div className="relative w-full max-w-md">
              <Input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search notifications" className="rounded-2xl" />
            </div>
          ...
          <CardContent className="space-y-2">
            {([
              ...
              <button
                ...
                className={`flex w-full items-center justify-between rounded-2xl border px-3 py-2 text-sm transition hover:bg-accent ${
                  category === entry ? "bg-accent" : ""
                }`}
              >
                <span>{entry}</span>
                  ...
                  return (
                    <React.Fragment key={item.id}>
                      <div className="flex gap-4 p-5 transition hover:bg-muted/40">
                        <div className="mt-1 rounded-2xl border p-2">
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          <div className="mt-4 flex flex-wrap gap-2">
                            ...
                            <Button
                              ...
                              onClick={() => deleteNotification(item.id).then(load)}
                            >
